#!/usr/bin/env bash
#
# Start the Oniro emulator in the background and wait until hdc can connect.
#
# Looks up to 10 ports above the requested port if the requested one is busy
# (matches hdc-py's behaviour for parallel local development).
#
# Required environment:
#   ONIRO_EMULATOR_PATH   Path to the extracted Oniro emulator images directory
#                         (the directory containing bzImage, ramdisk.img, ...).
#
# Optional arguments:
#   $1   Connect key in host:port form (default: 127.0.0.1:55555).
...
if [ ! -x "$launcher_script" ]; then
  echo "Error: expected executable launcher at $launcher_script" >&2
  exit 1
fi

if ! command -v hdc >/dev/null 2>&1; then
  echo "Error: hdc is not available on PATH." >&2
  exit 1
fi
...
if [ -f "$pid_file" ]; then
  ...
  if kill -0 "$existing_pid" >/dev/null 2>&1; then
    echo "Error: Oniro emulator appears to already be running with pid $existing_pid." >&2
    echo "Run 'just emulator-stop' first if you want to restart it." >&2
    exit 1
  fi
  rm -f "$pid_file"
fi
...
for port_offset in $(seq 0 9); do
  ...
  echo "Error: emulator process exited during startup." >&2
  echo "Check $log_file for emulator startup output." >&2
  exit 1
done

if [ -z "$actual_connect_key" ]; then
  echo "Error: failed to find an available host forwarding port starting at $requested_port." >&2
  echo "Check $log_file for emulator startup output." >&2
  exit 1
fi
...
for _ in $(seq 1 120); do
  if ! kill -0 "$emulator_pid" >/dev/null 2>&1; then
    echo "Error: emulator process exited before HDC became available." >&2
    echo "Check $log_file for emulator startup output." >&2
...
echo "Error: failed to connect to the Oniro emulator at $actual_connect_key." >&2
echo "Check $log_file for emulator startup output." >&2
rm -f "$pid_file" "$connect_file"
exit 1
